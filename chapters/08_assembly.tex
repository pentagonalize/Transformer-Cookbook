...
This is done via a layer-norm hash construction \WM{ref section}: we attend with the following head:
\begin{itemize}
    \item query $\phi(a_i/i, 1/i) = \phi(a_i)$;
    \item key $\phi(2j, 1) = \phi(2j)$;
    \item value $\phi(2j, 1) = \phi(2j)$.
\end{itemize}
Direct implementation of this head requires masked pre-norm, but it is possible it could be modified to not need this.
The last feedforward layer then verifies that the value retrieved by the head equals $\phi(a_i)$ and returns 1 iff so.
For further details of the construction and working with layer-norm hash, see \citet{merrill-2024-little-depth,merrill-sabharwal-2024-cot}.
